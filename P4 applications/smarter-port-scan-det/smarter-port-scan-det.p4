...
control MyIngress(inout headers hdr,
    ...
    register<bit<1>>(65535) flows_seen;
    // Alert bit
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            if(hdr.tcp.isValid()) {
			    ...
			    bit<16> new_destPort;
			    bit<1> current_flow_seen_before;
				...
				flows_seen.read(current_flow_seen_before, flow_hash_cast);
				if(current_flow_seen_before == 0) {
					//We haven't seen the flow of this packet before, record it, check lower ports for this flow
					flows_seen.write(flow_hash_cast, 1);
					...
					flow_hash_cast = (bit<32>)flow_hash;
					flows_seen.read(current_flow_seen_before, flow_hash_cast);
					if(current_flow_seen_before == 1) {
						//We've seen the lower port, keep checking
						flows_seen.write(flow_hash_cast, 1);

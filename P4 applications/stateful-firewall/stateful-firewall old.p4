...
control MyIngress(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata) {
    //register<bit<1>>(4096) known_flows; //track flows
	...
	action set_allowed(){
		...
		hash(meta.register_position_one, HashAlgorithm.crc16, (bit<32>)0, {hdr.ipv4.srcAddr,hdr.ipv4.dstAddr,
		hdr.tcp.srcPort,
		hdr.tcp.dstPort,
		hdr.ipv4.protocol},
    ...
    apply {
    	// meta.flow_id = hash(hdr.ipv4.srcAddr,
    	// 	hdr.ipv4.dstAddr,
    	// 	hdr.tcp.srcPort,
    	// 	hdr.tcp.dstPort,
    	// 	hdr.ipv4.protocol);
        if (hdr.ipv4.isValid()) {
            if (hdr.tcp.isValid()) {
                if (standard_metadata.ingress_port == 1) {
                	if (hdr.tcp.syn == 1) {
                		set_allowed(); //add to register if it is a syn packet from internal
                	}
                }

...
class ExerciseTopo(Topo):
    ...
    def __init__(self, hosts, switches, links, log_dir, **opts):
        ...
        for link in host_links:
            ...
            if (host_name == "h2"):
                self.addLink(host_name, host_sw, delay=link['latency'], bw=0.03,addr1=host_mac, addr2=host_mac)
            else:
...
class ExerciseRunner:
    ...
    def run_exercise(self):
        ...
        ax1.set_xlabel('Load level (packets/sec)')
        ax1.set_ylabel('Switch Register Flow Ratio Values', color='tab:red')
        load_level_list = [100, 125, 166, 250, 500, 1000]
        ax1.plot(load_level_list, p4_ratio_list, 'o', c='red', alpha=0.5, label='Flow ratio register values')
        ax1.tick_params(axis='y', labelcolor='tab:red')
        # ax1.set_xticks(load_level_list)
        
        ax2 = ax1.twinx()
        ax2.set_ylabel('hping3 Packet-Loss Percentages', color='tab:blue')
        ax2.plot(load_level_list, loss_percentage_list, 's', c='blue', alpha=0.5, label='hping3 reported packet loss percentages')
        ...
        ax2 = ax1.twinx()
        ax2.set_ylabel('hping3 Average Round-Trip-Time', color='tab:green')
        ax2.plot(load_level_list, average_rtt_list, '^', c='green', alpha=0.5, label='hping3 reported avg rtt')

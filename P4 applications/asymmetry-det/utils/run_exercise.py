...
def get_data(h1, child, p4_ratio_list, attack_flag_list, loss_percentage_list, average_rtt_list, cmd_string):
    h1.sendInt()
...
class ExerciseRunner:
    ...
    def run_exercise(self):
        ...
        while(timer < len(load_level_list)-1):
            sleep(10)
            print(timer)
        ...
        #Convert packets per sec to bytes
        load_level_list_in_bytes = [(1000000 / i) * 64 for i in load_level_list[:-1]]
        print(load_level_list_in_bytes)
        fig, ax1 = plt.subplots()
        plt.figure(1)

        ax1.set_xlabel('Offered Load (bytes/sec)')
        ax1.set_ylabel('Asymmetry', color='tab:red')
        ax1.plot(load_level_list_in_bytes, p4_ratio_list, 'o', c='red', alpha=0.5, label='Flow ratio register values')
        ax1.tick_params(axis='y', labelcolor='tab:red')
        # ax1.set_xticks(load_level_list)
        
        ax2 = ax1.twinx()
        ax2.set_ylabel('hping3 Packet-Loss Percentages', color='tab:blue')
        ax2.plot(load_level_list_in_bytes, loss_percentage_list, 's', c='blue', alpha=0.5, label='hping3 reported packet loss percentages')
        ax2.tick_params(axis='y', labelcolor='tab:blue')

        plt.title('Flow ratios and packet loss at different load levels')
        fig.tight_layout()
        ...
        ax2.set_ylabel('hping3 Average Round-Trip-Time (ms)', color='tab:green')
        ax2.plot(load_level_list_in_bytes, average_rtt_list, '^', c='green', alpha=0.5, label='hping3 reported avg rtt (log scale)')
        ax2.tick_params(axis='y', labelcolor='tab:green')
        # ax1.set_xticks(load_level_list)

        plt.title('Flow ratio and average rtt at different load levels')
        fig.tight_layout()

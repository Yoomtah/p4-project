...
class ExerciseRunner:
    ...
    def run_exercise(self):
        ...
        python_ratio_list = []
        p4_ratio_list = []
        flow_out_current_list = []
        flow_in_current_list = []
        flow_out_old_list = []
        flow_in_old_list = []
        attack_flag_list = []
        timer = 0
        ...
        while(timer < 9):
            child.sendline('register_read flow_out_to_other_host 1')
            child.readline()
            flow_out_current = float(child.readline().split('=')[1].strip())
            child.sendline('register_read flow_into_my_host 1')
            child.readline()
            flow_in_current = float(child.readline().split('=')[1].strip())
            child.sendline('register_read flow_out_to_other_host 0')
            child.readline()
            flow_out_old = float(child.readline().split('=')[1].strip())
            child.sendline('register_read flow_into_my_host 0')
            child.readline()
            flow_in_old = float(child.readline().split('=')[1].strip())
            ...
            print('Flow out current: ' + str(flow_out_current))
            print('Flow in current: ' + str(flow_in_current))
            print('Flow out old: ' + str(flow_out_old))
            print('Flow in old: ' + str(flow_in_old))
            print('P4 Flow Ratio: ' + str(p4_flow_ratio))
            print('Python Ratio: ' + str((flow_out_current - flow_out_old) / (flow_in_current - flow_in_old)))
            print('Attack flag: ' + str(attack_flag))
            flow_out_current_list.append(flow_out_current)
            flow_in_current_list.append(flow_in_current)
            flow_out_old_list.append(flow_out_old)
            flow_in_old_list.append(flow_in_old)
            p4_ratio_list.append(p4_flow_ratio)
            python_ratio_list.append((flow_out_current - flow_out_old) / (flow_in_current - flow_in_old))
        ...
        h3.sendInt()
        h3.waitOutput()
        h3.sendCmd('hping3 10.0.1.1 -i u100000', printPid=True)
        while(timer < 15):
            child.sendline('register_read flow_out_to_other_host 1')
            child.readline()
            flow_out_current = float(child.readline().split('=')[1].strip())
        ...
        plt.figure(1)
        plt.plot(range(0, 150, 10), flow_out_current_list, '-o', c='red', alpha=0.5, label='Count_packets_out_current register values')
        plt.title('Flow out packets recorded by switch over time')
        plt.xlabel('Time (10 sec incr)')
        plt.ylabel('Count_packets_out_current register value')
        plt.xscale('linear')
        plt.yscale('linear')
        plt.legend()
        plt.show()
        plt.figure(2)
        plt.plot(range(0, 150, 10), flow_in_current_list, '-o', c='blue', alpha=0.5, label='Count_packets_in_current register values')
        plt.title('Flow in packets recorded by switch over time')
        plt.xlabel('Time (10 sec incr)')
        plt.ylabel('Count_packets_in_current register value')
        plt.legend()
        plt.show()
        plt.figure(3)
        plt.plot(range(0, 150, 10), attack_flag_list, '-o', c='black', alpha=0.5, label='under_attack register values')
        plt.title('under_attack register values recorded by switch over time')
        plt.xlabel('Time (10 sec incr)')
        plt.ylabel('under_attack register value')
        plt.legend()
        plt.show()
        plt.figure(4)
        plt.plot(range(0, 150, 10), p4_ratio_list, '-o', c='green', alpha=0.5, label='flow_ratio register values')
        plt.title('Flow ratio recorded by switch and Python calculated ratios over time')
        plt.xlabel('Time (10 sec incr)')
        plt.ylabel('Ratio values')
        plt.plot(range(0, 150, 10), python_ratio_list, '-o', c='orange', alpha=0.8, label='Python calculated Count_packets_out/Count_packets_in values')

...
class ExerciseRunner:
    ...
    def run_exercise(self):
        ...
        h1.sendCmd('hping3 10.0.2.2 -i u10000', printPid=True)
        while(timer < 30):
            if timer == 5:
                h1.sendInt()
                h1.waitOutput()
        ...
        plt.figure(1)
        plt.plot(range(0, 300, 10), attack_flag_list, '-o', c='black', alpha=0.5, label='under_attack register values')
        ...
        plt.show()
        plt.figure(2)
        plt.plot(range(0, 300, 10), p4_ratio_list, '-o', c='green', alpha=0.5, label='flow_ratio register values')
